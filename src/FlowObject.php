...
class FlowObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = Util\Util::convertToFlowObject($v, $this->_opts);
    ...
    public function &__get($k)
    {
        ...
        if (!empty($this->_transientValues) && $this->_transientValues->includes($k)) {
            ...
            //Flow::getLogger()->error($message);

            return $nullval;
        }
        $class = static::class;
        //Flow::getLogger()->error("Stripe Notice: Undefined property of {$class} instance: {$k}");
    ...
    public function refreshFrom($values, $opts, $partial = false): void
    {
        ...
        if ($values instanceof FlowObject) {
    ...
    public function updateAttributes($values, $opts = null, $dirty = true): void
    {
        foreach ($values as $k => $v) {
            ...
            if (('metadata' === $k) && (\is_array($v))) {
                $this->_values[$k] = FlowObject::constructFrom($v, $opts);
            } else {
                $this->_values[$k] = Util\Util::convertToFlowObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParameters($force = false)
    {
        ...
        foreach ($this->_values as $k => $v) {
            ...
            if ($force || $unsaved || $v instanceof FlowObject) {
