...
abstract class Util
{
    ...
    /**
     * Converts a response from the Flow API to the corresponding PHP object.
     *
     * @param array $resp the response from the Flow API
     * @param array $opts
     *
     * @return array|FlowObject
     */
    public static function convertToFlowObject($resp, $opts)
    {
        $types = ObjectTypes::mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
                $mapped[] = self::convertToFlowObject($i, $opts);
            }
     ...
     *    it wasn't a string
     */
    public static function utf8($value): mixed
    {
        if (null === self::$isMbstringAvailable) {
            ...
            if (!self::$isMbstringAvailable) {
                \trigger_error('It looks like the mbstring extension is not enabled. ' .
                    ...
                    'support@up2date.ro if you have any questions.', \E_USER_WARNING);
            }
        }
