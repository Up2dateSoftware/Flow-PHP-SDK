...
use Up2date\FlowPhpSdk\Flow;
use Up2date\FlowPhpSdk\Util\CaseInsensitiveArray;
use Up2date\FlowPhpSdk\Util\RandomGenerator;
...
class CurlClient implements ClientInterface
{
     ...
     *   <li>CaseInsensitiveArray $rheaders The response headers</li>
    ...
    private function constructRequest($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if ('get' === $method) {
            ...
            $opts[\CURLOPT_POST] = 1;
            $opts[\CURLOPT_POSTFIELDS] = $hasFile ? $params : Util::encodeParameters($params);
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case \CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Flow ({$url}).  Please check your "
            ...
            case \CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify Flow's SSL certificate.  Please make sure "
        ...
        $msg .= ' let us know at support@up2date.ro.';
    ...
    {
        if ($numRetries >= Flow::getMaxNetworkRetries()) {
            return false;
        }
        ...
        if (isset($rheaders['flow-should-retry'])) {
            if ('false' === $rheaders['flow-should-retry']) {
                return false;
            }
            if ('true' === $rheaders['flow-should-retry']) {
                return true;
            }
        }
        ...
        //
        // Note that we expect the flow-should-retry header to be false
    ...
    private function sleepTime($numRetries, $rheaders)
    {
        ...
        $sleepSeconds = \min(
            Flow::getInitialNetworkRetryDelay() * 1.0 * 2 ** ($numRetries - 1),
            Flow::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = \max(Flow::getInitialNetworkRetryDelay(), $sleepSeconds);

        // And never sleep less than the time the API asks us to wait, assuming it's a reasonable ask.
        $retryAfter = isset($rheaders['retry-after']) ? (float) ($rheaders['retry-after']) : 0.0;
        if (\floor($retryAfter) === $retryAfter && $retryAfter <= Flow::getMaxRetryAfter()) {
